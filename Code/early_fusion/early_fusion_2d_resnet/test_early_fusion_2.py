...
def main():
    ...
    logging.info("==== TESTING VIDEO MODEL ====")
    logging.info(vars(args))

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # Preprocessing: resize->center-crop->tensor->normalize (r3d_18 stats)
    ...
    emotion_labels = {idx: cls for cls, idx in test_ds.class_to_idx.items()}
    ...
    with open(os.path.join(args.out_dir, 'misclassified.txt'), 'w') as f:
        for path, true, pred in misclassified:
            f.write(f"{path}: true={emotion_labels[true]}, pred={emotion_labels[pred]}\n")
