...
class CNNRNNDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clip = torch.stack(imgs, dim=0)  # shape: (T, C, H, W)
        return clip, label
...
class CNNRNN(nn.Module):
    def __init__(self, num_classes, hidden_size=256, num_layers=1):
        ...
        self.cnn = nn.Sequential(*list(cnn.children())[:-1])  # No FC
        self.feature_dim = 512
        self.rnn = nn.LSTM(input_size=self.feature_dim, hidden_size=hidden_size,
    ...
    def forward(self, x):  # x: (B, T, C, H, W)
        ...
        output, _ = self.rnn(feats)  # (B, T, H)
        return self.classifier(output[:, -1])  # Use last hidden state
...
def main():
    ...
    train_ds = CNNRNNDataset(args.data_root, 'train', args.num_frames, transform)
    val_ds = CNNRNNDataset(args.data_root, 'val', args.num_frames, transform)
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    model = CNNRNN(num_classes=len(train_ds.class_to_idx)).to(device)
