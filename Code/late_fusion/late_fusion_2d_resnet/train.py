...
class LateFusionResNet(nn.Module):
    ...
    def forward(self, x):  # x: (B, T, C, H, W)
        B, T, C, H, W = x.size()
        x = x.view(B * T, C, H, W)
        feats = self.encoder(x).view(B, T, -1)  # shape: (B, T, 512)
        agg = feats.mean(dim=1)  # Late fusion: average pooling over time
        return self.fc(agg)
...
def main():
    ...
    train_ds = LateFusionDataset(args.data_root, 'train', args.num_frames, transform)
    val_ds = LateFusionDataset(args.data_root, 'val', args.num_frames, transform)
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    model = LateFusionResNet(len(train_ds.class_to_idx)).to(device)
